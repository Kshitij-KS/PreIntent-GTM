...
// ─── TYPES ────────────────────────────────────────────────────────────────────
type View = "dashboard" | "signals" | "intel" | "brief" | "settings" | "knowledge";
...
// ─── HELPERS ─────────────────────────────────────────────────────────────────
function seedAccountToPremium(seed: {
  name: string;
  industry: string;
  employees: string;
  competitor: string;
...
function sweepResultToAccount(
  ...
  const mkEvidence = (title: string, event: string): EvidencePanel => ({
    ...
    capturedAt: now,
    details: [event, `Score: ${result.convergenceScore}/100`, `Urgency: ${result.urgency}`],
  });
    ...
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle
          ...
          fill="none" stroke={`url(#rg${value})`} strokeWidth="6"
...
const AddAccountModal = ({
  ...
  isOpen: boolean;
  onClose: () => void;
  onAdd: (data: { name: string; industry: string; employees: number; competitor: string; competitorPricingUrl: string }) => void;
}) => {
  const [form, setForm] = useState({ name: "", industry: "", employees: "500", competitor: "", competitorPricingUrl: "" });
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => { setAdding(false); onClose(); setForm({ name: "", industry: "", employees: "500", competitor: "", competitorPricingUrl: "" }); }, 800);
  };
      ...
      >
        ...
        <div style={{ display: "grid", gap: "16px" }}>
          {[
            ...
            { label: "INDUSTRY", key: "industry", placeholder: "FinTech / Payments" },
            { label: "EMPLOYEE COUNT", key: "employees", placeholder: "500", type: "number" },
            { label: "PRIMARY COMPETITOR *", key: "competitor", placeholder: "Stripe" },
            { label: "COMPETITOR PRICING URL", key: "competitorPricingUrl", placeholder: "https://stripe.com/pricing" },
          ].map(({ label, key, placeholder, type }) => (
            <div key={key}>
              <div style={{ fontSize: "10px", color: C.muted, letterSpacing: "0.08em", marginBottom: "6px" }}>{label}</div>
              <input
                type={type || "text"}
                ...
                style={{
                  ...
                  borderRadius: "8px", padding: "10px 12px", fontSize: "13px", color: C.text,
        ...
        <div style={{ display: "flex", gap: "10px", marginTop: "24px" }}>
          <button
            ...
            style={{
              ...
              borderRadius: "8px", padding: "11px", fontSize: "12px", color: C.muted,
          ...
          <button
            ...
            style={{
              flex: 2, background: adding ? C.dim : `linear-gradient(135deg, #7c3aed, ${C.conv})`,
              border: "none", borderRadius: "8px", padding: "11px", fontSize: "12px",
              color: C.white, cursor: adding ? "not-allowed" : "pointer", fontFamily: "inherit",
...
const KnowledgeView = ({ doc }: { doc: CompanyKnowledgeDoc | null }) => {
  if (!doc) {
    return (
      <div style={{ padding: "48px", textAlign: "center", color: C.muted }}>
        ...
        <div style={{ fontSize: "11px", marginTop: "8px" }}>Complete onboarding to generate your company intelligence document.</div>
        <a href="/onboarding" style={{
          display: "inline-block", marginTop: "16px", color: C.conv,
          textDecoration: "none", border: `1px solid ${C.conv}`, borderRadius: "6px", padding: "8px 16px", fontSize: "11px",
        }}>
          Complete Onboarding →
        </a>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: "18px" }}>
      {/* Header */}
      <div style={{
        background: `${C.conv}12`, border: `1px solid ${C.conv}30`,
        ...
        <p style={{ margin: 0, fontSize: "12px", color: C.text, lineHeight: 1.8 }}>
          {doc.segmentSummary}
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "14px" }}>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "16px" }}>
          <div style={{ fontSize: "9px", color: C.conv, fontWeight: 700, letterSpacing: "0.1em", marginBottom: "12px" }}>IDEAL CUSTOMER PROFILE</div>
          <div style={{ display: "grid", gap: "8px" }}>
            <div>
              <div style={{ fontSize: "9px", color: C.muted, marginBottom: "3px" }}>COMPANY SIZE</div>
              <div style={{ fontSize: "12px", color: C.text }}>{doc.icp.companySize}</div>
            </div>
            <div>
              <div style={{ fontSize: "9px", color: C.muted, marginBottom: "5px" }}>TARGET INDUSTRIES</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                {doc.icp.industries.slice(0, 4).map((ind) => (
                  <span key={ind} style={{ fontSize: "10px", padding: "2px 7px", background: `${C.blue}12`, border: `1px solid ${C.blue}25`, borderRadius: "4px", color: C.blue }}>
                    {ind}
                  </span>
                ))}
              </div>
            ...
            <div>
              <div style={{ fontSize: "9px", color: C.muted, marginBottom: "5px" }}>TRIGGER EVENTS</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                {doc.icp.triggerEvents.slice(0, 3).map((t) => (
                  <span key={t} style={{ fontSize: "10px", padding: "2px 7px", background: `${C.pain}12`, border: `1px solid ${C.pain}25`, borderRadius: "4px", color: C.pain }}>
                    {t}
                  </span>
                ))}
              </div>
        ...
        {/* Scan Config */}
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "16px" }}>
          <div style={{ fontSize: "9px", color: C.void, fontWeight: 700, letterSpacing: "0.1em", marginBottom: "12px" }}>SCAN CONFIGURATION</div>
          <div style={{ display: "grid", gap: "10px" }}>
            <div>
              <div style={{ fontSize: "9px", color: C.muted, marginBottom: "5px" }}>COMPETITORS TRACKED</div>
              {doc.scanConfig.competitors.map((c) => (
                <div key={c} style={{ fontSize: "11px", color: C.void, display: "flex", alignItems: "center", gap: "6px", padding: "2px 0" }}>
                  <span style={{ fontSize: "8px" }}>▸</span> {c}
                </div>
              ))}
            </div>
            <div>
              <div style={{ fontSize: "9px", color: C.muted, marginBottom: "5px" }}>REGULATORY KEYWORDS</div>
              {doc.scanConfig.regulatoryKeywords.slice(0, 3).map((k) => (
                <div key={k} style={{ fontSize: "10px", color: C.compliance, padding: "1px 0" }}>{k}</div>
              ))}
            </div>
          </div>
        </div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "16px", marginBottom: "14px" }}>
        ...
        {doc.opportunities.map((opp, i) => {
          const urgColor = opp.urgency === "HIGH" ? C.void : opp.urgency === "MEDIUM" ? C.compliance : C.muted;
          return (
            <div key={i} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                <span style={{ fontSize: "12px", fontWeight: 600, color: C.white }}>{opp.title}</span>
                <span style={{
                  fontSize: "8px", padding: "2px 7px", borderRadius: "3px", letterSpacing: "0.08em",
                  background: `${urgColor}20`, color: urgColor,
                }}>{opp.urgency}</span>
              </div>
              <p style={{ margin: "0 0 6px", fontSize: "11px", color: C.muted, lineHeight: 1.6 }}>{opp.description}</p>
              <div style={{ fontSize: "10px", color: C.conv }}>→ {opp.suggestedAction}</div>
            </div>
          );
        })}
      </div>
      ...
      {doc.seedAccounts.length > 0 && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "16px" }}>
          <div style={{ fontSize: "9px", color: C.void, fontWeight: 700, letterSpacing: "0.1em", marginBottom: "14px" }}>SEED ACCOUNTS FROM ONBOARDING</div>
          {doc.seedAccounts.map((acc, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: i < doc.seedAccounts.length - 1 ? `1px solid ${C.border}` : "none" }}>
              <div>
                <div style={{ fontSize: "12px", color: C.white, fontWeight: 500 }}>{acc.name}</div>
                <div style={{ fontSize: "10px", color: C.muted, marginTop: "2px" }}>{acc.industry} · {acc.employees}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: "10px", color: C.void }}>{acc.competitor}</div>
                <div style={{ fontSize: "10px", color: C.muted, maxWidth: "200px", textAlign: "right", marginTop: "2px", lineHeight: 1.4 }}>{acc.whyNow}</div>
              </div>
            </div>
          ))}
        </div>
...
const SignalsView = ({ accounts, onEvidence, filter, onFilterChange }: {
  ...
  type SignalItem = { account: PremiumAccount; type: "void" | "compliance" | "pain"; score: number; event: string; time: string; color: string };
        ...
        {(["all", "void", "compliance", "pain"] as const).map((f) => (
          <button
            ...
            style={{
              ...
              fontFamily: "inherit", letterSpacing: "0.08em", cursor: "pointer", transition: "all 0.15s",
            }}
          >
            {f.toUpperCase()}
          </button>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
        {allSignals.map((s, i) => (
          <motion.div
            ...
            style={{
              border: `1px solid ${C.border}`, borderRadius: "7px", padding: "12px 14px",
          ...
          >
            ...
            <div>
              <div style={{ fontSize: "11px", color: C.white, fontWeight: 500, marginBottom: "2px" }}>{s.account.name}</div>
              <div style={{ fontSize: "10px", color: C.muted }}>{s.event}</div>
            </div>
            <div style={{ fontSize: "22px", fontWeight: 700, color: s.color, textAlign: "right" }}>{s.score}</div>
            <div style={{ fontSize: "9px", color: C.muted, textAlign: "right" }}>{s.time}</div>
          </motion.div>
        ))}
        {allSignals.length === 0 && (
          <div style={{ textAlign: "center", padding: "40px", color: C.muted, fontSize: "12px" }}>
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: "18px" }}>
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px", flexShrink: 0 }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: "9px", color: C.muted, marginTop: "5px" }}>vs {account.competitor}</div>
        </div>
      </div>
    </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: account.audioEvidence ? "1fr 1fr 1fr 1fr" : "1fr 1fr 1fr", gap: "8px", marginBottom: "12px" }}>
      {[
        ...
        ...(account.audioEvidence ? [{ type: "audio" as const, label: "AUDIO INTEL", score: 88, event: account.audioSignal ?? "Podcast signal detected", time: account.audioEvidence?.capturedAt ?? account.lastUpdated, color: "#c084fc", conf: 0.89 }] : []),
      ].map((sig) => (
        <motion.div
          key={sig.type}
...
const SettingsView = ({
  ...
  const engineStatus = integrationStatuses.length > 0
    ? integrationStatuses.map((s) => {
        ...
        return { name: s.name, tag: meta.tag, color: meta.color, live: s.status === "live", mode: s.mode ?? "mock", detail: s.detail ?? "" };
      })
    : [
        { name: "Bright Data", tag: "Void Scanner · Compliance Radar · Pain Listener", color: "#00aaff", live: false, mode: "mock", detail: "Set BRIGHT_DATA_MODE=real + BRIGHT_DATA_API_KEY" },
        { name: "AI/ML API", tag: "Signal scoring + Intel Brief generation", color: "#ff5a52", live: false, mode: "mock", detail: "Set AI_ML_MODE=real + AI_ML_API_KEY" },
        { name: "Featherless AI", tag: "Pain signal classification (Mistral-7B)", color: "#24c038", live: false, mode: "mock", detail: "Set FEATHERLESS_MODE=real + FEATHERLESS_API_KEY" },
        { name: "Speechmatics", tag: "Podcast & audio transcription", color: "#f0a000", live: false, mode: "mock", detail: "Set SPEECHMATICS_MODE=real + SPEECHMATICS_API_KEY" },
        { name: "Cognee", tag: "Account memory graph (knowledge persistence)", color: "#9060ff", live: true, mode: "real", detail: "Browser localStorage (zero-config)" },
        { name: "TriggerWare", tag: "Workflow routing to Slack + HubSpot", color: "#ff8800", live: false, mode: "mock", detail: "Set TRIGGERWARE_MODE=real + TRIGGERWARE_WEBHOOK_URL" },
        { name: "Slack", tag: "Real-time Slack alerts on convergence threshold", color: "#4A154B", live: false, mode: "mock", detail: "Set SLACK_MODE=real + SLACK_WEBHOOK_URL" },
        { name: "HubSpot", tag: "CRM lead creation on convergence breach", color: "#FF7A59", live: false, mode: "mock", detail: "Set HUBSPOT_MODE=real + HUBSPOT_WEBHOOK_URL" },
      ];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: "18px" }}>
      {/* Account info */}
      {userEmail && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "14px 16px", marginBottom: "12px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: "9px", color: C.muted, letterSpacing: "0.1em", marginBottom: "4px" }}>SIGNED IN AS</div>
            ...
            <div key={e.name} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <div>
                  ...
                  {!e.live && e.detail && <div style={{ fontSize: "9px", color: C.dim, marginTop: "2px" }}>{e.detail}</div>}
                </div>
              </div>
              <span style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px", marginBottom: "12px" }}>
        {[
          ...
          { label: "PLAN", value: "Elite", sub: "All 3 engines", color: C.blue },
        ].map((s) => (
...
export default function RealDashboard({
  ...
  const runScan = useCallback(async () => {
    ...
    toast.info("Full scan initiated", `Monitoring ${accounts.length} accounts across 3 engines`);
      ...
      try {
        ...
        return {
          ...
          result: {
            ...
            complianceEvent: profile.compliance?.signals?.[0]?.title ?? account.complianceEvent,
    ...
    const topResult = Array.from(resultMap.values()).sort((a, b) => b.convergenceScore - a.convergenceScore)[0];
    const slackSent = Array.from(resultMap.values()).some((r) => r.slackSent);
    const hubspotSent = Array.from(resultMap.values()).some((r) => r.hubspotSent);

    const highAlerts = Array.from(resultMap.values()).filter((r) => r.convergenceScore >= 85).length;
    if (highAlerts > 0) {
      setTriggerFired(true);
      toast.convergenceAlert(accounts[0]?.name ?? "Account", topResult?.convergenceScore ?? 85);
      if (slackSent) setTimeout(() => toast.triggerWareFired(accounts[0]?.name ?? "Account"), 900);
    }
  ...
  const handleAddAccount = useCallback(async (data: {
    name: string; industry: string; employees: number; competitor: string; competitorPricingUrl: string;
  }) => {
    ...
    const newAccount: PremiumAccount = {
      ...
      voidScore: 0, voidConfidence: 0,
      complianceScore: 0, complianceConfidence: 0,
      painScore: 0, painConfidence: 0,
      convergence: 0, overallConfidence: 0,
      status: "MONITOR" as const,
      contact: { title: "Decision Maker", name: data.name + " Contact", linkedin: "" },
      stack: [],
      ...
      if (res.ok) {
        ...
        if (result.success && result.profile) {
          ...
          const sweepRes: SweepResult = {
            ...
            complianceScore: profile.compliance?.subScore ?? 0,
            painScore: profile.pain?.subScore ?? 0,
            voidEvent: profile.void?.signals?.[0]?.title ?? newAccount.voidEvent,
            complianceEvent: profile.compliance?.signals?.[0]?.title ?? newAccount.complianceEvent,
            painEvent: profile.pain?.signals?.[0]?.title ?? newAccount.painEvent,
            slackSent: result.slackSent,
            hubspotSent: result.hubspotSent,
          };
          setAccounts((prev) => prev.map((a) => a.id === newId ? sweepResultToAccount(a, sweepRes) : a));
          toast.info(`${data.name} sweep complete`, `Convergence: ${profile.convergenceScore}/100`);
...
${realBrief.whyNow?.map((w, i) => `[${["VOID", "COMPLIANCE", "PAIN"][i] || w.engine.toUpperCase()}] Sub-score ${w.subScore}/100\n${w.narrative}`).join("\n\n") ?? ""}`;
  ...
  const liveApisCount = liveIntegrations.filter((i) => i.status === "live").length;
    ...
    <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: "18px" }}>
      ...
      {!setupComplete && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          style={{
            background: `${C.conv}10`, border: `1px solid ${C.conv}35`,
            borderRadius: "8px", padding: "14px 18px", marginBottom: "14px",
            display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px",
          }}
        >
          <div>
            <div style={{ fontSize: "12px", fontWeight: 600, color: C.white, marginBottom: "3px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px", marginBottom: "14px" }}>
        ...
        <StatCard label="LIVE APIs" value={`${liveApisCount}`} sub="of 8 integrations" color={C.conv} delay={3} onClick={() => setView("settings")} />
      </div>
      ...
      {knowledgeDoc && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          style={{
            background: C.surface, border: `1px solid ${C.conv}25`,
            borderRadius: "8px", padding: "12px 16px", marginBottom: "14px",
            display: "flex", alignItems: "center", gap: "16px", flexWrap: "wrap",
          }}
        >
          <div style={{ fontSize: "9px", color: C.conv, letterSpacing: "0.1em", flexShrink: 0 }}>YOUR COMPANY</div>
          <div style={{ fontSize: "12px", color: C.white, fontWeight: 600 }}>{knowledgeDoc.companyName}</div>
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
            {knowledgeDoc.scanConfig.competitors.slice(0, 3).map((c) => (
              <span key={c} style={{ fontSize: "9px", padding: "2px 7px", background: `${C.void}12`, border: `1px solid ${C.void}25`, borderRadius: "3px", color: C.void }}>
                vs {c}
              </span>
            ))}
          </div>
          <button
            ...
            style={{
              ...
              borderRadius: "5px", padding: "4px 12px", fontSize: "10px", color: C.muted,
              cursor: "pointer", fontFamily: "inherit",
            }}
          >
            View Knowledge Doc →
      ...
      <div style={{ display: "flex", gap: "10px", marginTop: "12px" }}>
        <button
          ...
          onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.conv; e.currentTarget.style.color = C.conv; }}
          onMouseLeave={(e) => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.color = C.muted; }}
        >
        ...
        padding: "0 18px", gap: "0", flexShrink: 0,
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
            ...
            <button
              ...
              style={{
                display: "flex", alignItems: "center", gap: "5px",
                background: `${C.conv}15`, border: `1px solid ${C.conv}35`,
                borderRadius: "5px", padding: "4px 10px", fontSize: "9px",
                color: C.conv, cursor: "pointer", fontFamily: "inherit",
              }}
            >
              <Icon.Plus /> Add Account
            </button>

            <button onClick={handleSignOut} style={{ ...ghostBtnStyle, fontSize: "9px", padding: "4px 10px" }}>
              Sign out
            </button>
          </div>
        </div>
