...
import {
  ...
  ComparisonTrigger,
} from "@/components/ui/competitive-comparison";
import {
  formatRelativeTime,
  getConfidenceLevel,
  type PremiumAccount,
...
const SCAN_STEPS = [
  ...
  {
    msg: "Featherless AI — classifying pain signals (Mistral-7B)...",
    tag: "Featherless AI",
    pct: 68,
  },
  {
...
export default function RealDashboard({
  ...
  const runScan = useCallback(async () => {
    ...
    const runAccountSweep = async (account: PremiumAccount) => {
      ...
      const competitor =
        account.competitor || doc?.scanConfig.competitors[0] || "Competitor";
      ...
      try {
        const res = await fetch("/api/sweep", {
          ...
          body: JSON.stringify({
            ...
            competitor,
            competitorPricingUrl: account.competitorUrl
        ...
        {accounts.map((a, idx) => (
          <motion.div
            ...
            style={{
              ...
              alignItems: "center",
              borderBottom:
                idx < accounts.length - 1 ? `1px solid ${C.border}` : "none",
              cursor: "pointer",
            }}
